...
with st.sidebar:
    ...
    if is_connected:
        st.markdown(
            'Trạng thái: <span class="status-connected">● Đã liên kết thành công</span>', unsafe_allow_html=True)
        if st.button("Ngắt kết nối Gmail", use_container_width=True):
            if os.path.exists(TOKEN_PATH):
                os.remove(TOKEN_PATH)
            st.toast("Đã hủy quyền truy cập Gmail!")
        ...
        st.markdown(
            'Trạng thái: <span class="status-disconnected">● Chưa được cấp quyền</span>', unsafe_allow_html=True)
        if st.button("Kết nối với Google Gmail", type="primary", use_container_width=True):
            with st.spinner("Đang mở trình duyệt để xác thực OAuth2..."):
                try:
                    authenticate_gmail_flow()
...
with col_chat:
    ...
    # IMPROVEMENT: Bộ lọc hiển thị lịch sử chat (chỉ hiện text sạch của User và Assistant, ẩn log gọi tool thô)
    for message in st.session_state.messages:
        if message["role"] == "user" or (message["role"] == "assistant" and message.get("content")):
            with st.chat_message(message["role"]):
                st.markdown(message["content"])
    ...
    if user_query:
        ...
        # IMPROVEMENT: Lưu câu hỏi của User vào bộ nhớ trước khi xử lý để làm dày Context
        st.session_state.messages.append(
            {"role": "user", "content": user_query})

        with st.spinner("Agent đang suy nghĩ và điều phối công cụ..."):
            try:
                # IMPROVEMENT: Truyền toàn bộ mảng lịch sử hội thoại thay vì truyền một chuỗi text đơn lẻ
                output = process_user_prompt(st.session_state.messages)
                final_answer = output["final_answer"]
                tool_calls = output["tool_calls"]
                tool_results = output["tool_results"]
                new_messages = output.get("new_messages", [])
            except Exception as e:
                final_answer = f"Hệ thống gặp lỗi: {str(e)}"
                tool_calls, tool_results = [], []
                new_messages = [{"role": "assistant", "content": final_answer}]

        with st.chat_message("assistant"):
            st.markdown(final_answer)

        # IMPROVEMENT: Nạp toàn bộ chuỗi hội thoại sinh ra ở lượt này (gồm các cặp phản hồi gọi hàm nếu có) vào Memory
    ...
    if st.session_state.email_draft:
        ...
        with st.form("email_confirmation_form"):
            ...
            edit_recipient = st.text_input(
                "Người nhận", value=draft.get("recipient", ""))
            edit_subject = st.text_input(
                "Tiêu đề", value=draft.get("subject", ""))
            edit_body = st.text_area(
                "Nội dung", value=draft.get("body", ""), height=200)

            col_submit, col_cancel = st.columns([1, 1])
            with col_submit:
                submitted = st.form_submit_button(
                    "Xác nhận Gửi", type="primary", use_container_width=True)
            with col_cancel:
                cancelled = st.form_submit_button(
                    "Hủy bỏ", use_container_width=True)
    ...
    if st.session_state.show_logs:
        if st.session_state.logs:
            ...
            if tool_calls:
                ...
                for i, (tc, tr) in enumerate(zip(tool_calls, tool_results)):
                    st.markdown(
                        f"**[Tool Call {i+1}] Tên Tool:** `{tc['name']}`")
                    st.json(tc['arguments'])
                    st.markdown("Tool được thực thi và trả về kết quả:")
                    st.info(f"Dữ liệu Tool: {tr['output']}")
            else:
                st.warning(
                    "Câu hỏi này LLM tự suy luận trực tiếp, không cần kích hoạt Tool.")
        else:
            st.info("Chưa có log nào. Hãy gửi câu hỏi để bắt đầu!")
