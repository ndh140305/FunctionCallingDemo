...
tools = [
    {"type": "function", "function": weather_tool_declaration},
    {"type": "function", "function": math_tool_declaration},
    {"type": "function", "function": pdf_tool_declaration},
    {"type": "function", "function": coords_tool_declaration},
    {"type": "function", "function": email_draft_declaration}
]

# IMPROVEMENT: Router nhận mảng chat_history để phân loại ngữ cảnh chính xác ở các câu hỏi tiếp nối


def classify_intent(chat_history: list) -> str:
    router_prompt = """Bạn là một bộ định tuyến (Router). 
    Nhiệm vụ của bạn là đọc toàn bộ lịch sử hội thoại và phân loại CÂU HỎI MỚI NHẤT của người dùng vào các nhóm sau. 
    Nếu câu hỏi có nhiều yêu cầu, hãy trả về TẤT CẢ các từ khóa tương ứng, cách nhau bằng dấu phẩy. 
    Tuyệt đối CHỈ TRẢ VỀ CÁC TỪ KHÓA, không giải thích gì thêm:

- THOI_TIET: Nếu câu hỏi mới nhất hỏi về thời tiết, nhiệt độ (hoặc đại từ thay thế liên quan đến ngữ cảnh thời tiết phía trên).
- TOAN_HOC: Nếu câu hỏi chứa phép tính toán học.
- DOC_PDF: Nếu người dùng yêu cầu đọc tài liệu, tóm tắt file PDF.
- EMAIL: Nếu người dùng yêu cầu soạn hoặc gửi email.
- KIEN_THUC_CHUNG: Các câu hỏi giao tiếp bình thường, hỏi kiến thức chung (VD: xin chào, quả chuối màu gì...)"""

    # Gửi kèm lịch sử hội thoại cho Router
    messages = [{"role": "system", "content": router_prompt}] + chat_history
...
# IMPROVEMENT: Hàm nhận vào chat_history thay vì một chuỗi text cô lập


def process_user_prompt(chat_history: list) -> dict:
    """Xử lý prompt của user qua Groq Function Calling có giữ ngữ cảnh hội thoại."""

    user_prompt = chat_history[-1]["content"] if chat_history else ""
    intent = classify_intent(chat_history).upper()
    print(f"Router phân loại dựa trên ngữ cảnh: {intent}")
    ...
    if "TOAN_HOC" in intent:
        tools_to_use.append(
            {"type": "function", "function": math_tool_declaration})
        system_prompts.append(
            "Hãy dùng tool calculate_expression để tính toán chính xác.")
    ...
    # IMPROVEMENT: Khởi tạo mảng chạy cục bộ bằng cách nối System Prompt mới vào đầu lịch sử hội thoại đã có
    ...
    result = {
        ...
        "steps": [],
        "final_answer": None,
        # IMPROVEMENT: Gom cụm toàn bộ payload sinh ra ở lượt này để nạp vào session_state ngoài app.py
        "new_messages": []
    }
    ...
    while iteration < max_iterations:
        try:
            ...
            if "400" in error_str and "tool" in error_str.lower():
                try:
                    ...
                    result["final_answer"] = response_fallback.choices[0].message.content + \
                        "\n\n(Lưu ý: Hệ thống gặp khó khăn khi gọi công cụ, câu trả lời dựa trên kiến thức gốc của AI.)"
                    ...
                    result[
                        "final_answer"] = f"Lỗi hệ thống khi gọi AI (Fallback failed): {str(fallback_error)}"
                    result["new_messages"].append(
                        {"role": "assistant", "content": result["final_answer"]})
                    return result
            result["final_answer"] = f"Hệ thống gặp lỗi API: {error_str}"
        ...
        # Trường hợp không cần gọi thêm tool nào nữa -> Trả về câu trả lời cuối cùng
        if not getattr(response_message, "tool_calls", None):
            result["final_answer"] = response_message.content
            result["new_messages"].append(
        ...
        # Trường hợp LLM yêu cầu gọi Tool
        assistant_msg = {
            "role": "assistant",
            "content": response_message.content,
            "tool_calls": [
        ...
        has_tool_error = False
        with concurrent.futures.ThreadPoolExecutor() as executor:
            futures = [executor.submit(execute_tool_parallel, tc)
                       for tc in response_message.tool_calls]
            for future in concurrent.futures.as_completed(futures):
                t_id, function_name, function_args, tool_output = future.result()
                ...
                result["tool_results"].append(
                    {"name": function_name, "output": tool_output})
                result["steps"].append(
                    {"tool": function_name, "output": tool_output})
        ...
        if has_tool_error:
            sys_err_msg = {
                "role": "system",
                "content": "Công cụ vừa gọi trả về lỗi. DỪNG gọi thêm công cụ. Hãy thông báo lỗi này cho người dùng ngay lập tức.",
            }
            messages.append(sys_err_msg)
            result["new_messages"].append(sys_err_msg)
